...
const parseCallouts = (html: string) => {
  ...
  // Improved regex to catch the blockquote structure generated by marked
  return html.replace(/<blockquote>\s*<p>\[!(INFO|WARNING|ATTENTION|TIP|ASTUCE|SUCCESS|SUCCÈS)\](.*?)<\/p>(.*?)<\/blockquote>/gs, (match, type, title, body) => {
    const config = calloutMap[type.toUpperCase()];
    return `<div class="callout ${config.class}">
      <div class="callout-header">${config.icon} ${config.label}${title ? ' : ' + title.trim() : ''}</div>
      <div class="callout-content">${body.trim()}</div>
    </div>`;
  });
};
...
const WikiPage: React.FC = () => {
    ...
    if (range) {
      const tableHTML = `
        <table style="width:100%; border-collapse: collapse;">
          ...
          <tbody>
            <tr><td>Donnée</td><td>Donnée</td></tr>
          </tbody>
        </table><p><br></p>
      `;
  ...
  const insertCallout = (type: string) => {
    ...
    const range = quill.getSelection();
    if (range) {
      const calloutHTML = `<blockquote><p>[!${type}] Titre du bloc</p><p>Votre texte ici...</p></blockquote><p><br></p>`;
                    ...
                    <div className="editor-side visual-side" 
                      ...
                      <div className="quill-wrapper" ref={wrapperRef} style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                        <div className="custom-quill-toolbar">
                            ...
                            <div className="toolbar-separator-mini"></div>
                            <button onClick={insertTable} title="Insérer un tableau"><Table size={16}/></button>
                            <button onClick={insertHR} title="Ligne de séparation"><Minus size={16}/></button>
                            <div className="toolbar-separator-mini"></div>
                            <button onClick={() => insertCallout('INFO')} title="Bloc Info" style={{color: '#3b82f6'}}><Info size={16}/></button>
                            <button onClick={() => insertCallout('WARNING')} title="Bloc Attention" style={{color: '#f59e0b'}}><AlertTriangle size={16}/></button>
                            <button onClick={() => insertCallout('TIP')} title="Bloc Astuce" style={{color: '#8b5cf6'}}><Lightbulb size={16}/></button>
                            <button onClick={() => insertCallout('SUCCESS')} title="Bloc Succès" style={{color: '#10b981'}}><CheckCircle size={16}/></button>
                            <div className="toolbar-separator-mini"></div>
